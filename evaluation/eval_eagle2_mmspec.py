"""EAGLE2 speculative decoding evaluation for MMSpec unified dataset.

EAGLE2 uses dynamic confidence-aware tree construction instead of EAGLE1's fixed tree.
Same draft model checkpoint, different inference strategy.

Usage:
python -m evaluation.eval_eagle2_mmspec --base-model-path <path> --spec-model-path <path> --data-folder <path>
"""
...
from method.eagle2.kv_cache import initialize_past_key_values
from method.eagle2.utils import *
from method.eagle2.spec_model import SpecModel  # EAGLE2 SpecModel with dynamic tree
...
def evaluate(args):
    """Run EAGLE2 speculative decoding evaluation on MMSpec dataset."""
    # Load model
...
def main():
    parser = argparse.ArgumentParser(description="EAGLE2 evaluation for MMSpec")
    parser = get_common_args(parser)
    
    # EAGLE2 specific args
    parser.add_argument(
        "--total-token",
        type=int,
        default=30,
        help="Total tokens for speculative decoding tree",
    )
    parser.add_argument(
        "--depth",
        type=int,
        default=3,
        help="Maximum depth for dynamic tree expansion",
    )
    parser.add_argument(
        "--top-k",
        type=int,
        default=8,
        help="Top-k candidates per node expansion",
    )
    parser.add_argument(
        "--threshold",
        type=float,
        default=1.0,
        help="Confidence threshold for EAGLE2 dynamic tree (log-prob). "
             "Nodes with cumulative score below log(threshold) are pruned.",
    )
    ...
    args.model_id = args.model_id + "-eagle2-temperature-" + str(args.temperature)
    ...
    print("Method: EAGLE2 (Dynamic Tree Speculative Decoding)")
