...
Usage:
python -m evaluation.eval_lookahead_mmspec --base-model-path <path> --data-folder <path>
"""
...
from method.lookahead.spec_model_lookahead import SpecModel
...
def evaluate(args):
    """Run Lookahead evaluation on MMSpec dataset."""
    attn_impl = args.attn_implementation
    if attn_impl == "sdpa":
        print("[Lookahead] attn_implementation=sdpa requested, but Lookahead disables SDPA for stable behavior. Forcing eager.")
        attn_impl = "eager"
...
def main():
    parser = argparse.ArgumentParser(description="Lookahead speculative decoding evaluation for MMSpec")
    parser = get_common_args(parser)
    
    # Lookahead specific args
    parser.add_argument(
        "--decoding-length",
        type=int,
        default=64,
        help="Decoding length for lookahead",
    )
    parser.add_argument(
        "--branch-length",
        type=int,
        default=12,
        help="Branch length for lookahead",
    )
    parser.add_argument(
        "--attn-implementation",
        type=str,
        ...
        help="HF attention implementation. Lookahead forcibly disables sdpa and uses eager.",
    )
    ...
    args.model_id = args.model_id + "-lookahead-temperature-" + str(args.temperature)
    ...
    print("Method: Lookahead")
    print(f"Lookahead params: decoding_length={args.decoding_length}, branch_length={args.branch_length}")
