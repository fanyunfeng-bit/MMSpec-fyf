"""SAM (visual-token skip) speculative decoding evaluation for MMSpec unified dataset.

Usage:
python -m evaluation.eval_sam_vskip_mmspec --base-model-path <path> --data-folder <path>
"""
...
from method.sam_vskip.spec_model import SpecModel
...
def evaluate(args):
    """Run SAM-vskip evaluation on MMSpec dataset."""
    attn_impl = args.attn_implementation
    if attn_impl == "sdpa":
        print("[SAM-vskip] attn_implementation=sdpa requested, but vskip needs attentions and SDPA may skip them. Forcing eager.")
        attn_impl = "eager"
    ...
    for _ in range(3):
        ...
        model.specgenerate(
            **model_inputs,
            ...
            visual_probe_only=args.visual_probe_only,
        )
    print("Warmup done")
...
def main():
    parser = argparse.ArgumentParser(description="SAM-vskip speculative decoding evaluation for MMSpec")
    ...
    parser.add_argument(
        "--disable-visual-skip",
        action="store_true",
        help="Disable visual-token skip fallback and run original SAM everywhere",
    )
    parser.add_argument(
        "--visual-attn-threshold",
        type=float,
        default=0.35,
        help="When current decode-step attention mass on visual tokens >= this threshold, force target-only next token from same forward pass.",
    )
    parser.add_argument(
        "--visual-probe-only",
        action="store_true",
        help="Only measure visual attention; do not alter SAM acceptance behavior.",
    )
    parser.add_argument(
        "--attn-implementation",
        type=str,
        ...
        help="HF attention implementation. SAM-vskip forcibly disables sdpa and uses eager.",
    )

    args = parser.parse_args()

    args.model = args.base_model_path
    args.model_id = args.model_id + "-sam-vskip-temperature-" + str(args.temperature)
