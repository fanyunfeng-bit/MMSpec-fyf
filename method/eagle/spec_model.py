...
def _detect_checkpoint_arch_from_state_dict(state_dict):
    """Detect if a loaded state_dict uses EAGLE3 architecture (midlayer keys)."""
    for k in state_dict:
        if "midlayer." in k:
            return "eagle3"
    return "eagle1"
...
def _normalize_eagle3_checkpoint_keys(state_dict):
    """Normalize eagle3 checkpoint keys (strip module./model. prefixes)."""
    normalized = {}
...
def _pick_default_spec_config(base_model_path, ckpt_arch="eagle1"):
    """Pick a local fallback spec config that matches the base model family."""
    try:
    ...
    if base_cfg is not None:
        ...
        arch = architectures[0] if architectures else getattr(base_cfg, "model_type", "")
    arch = arch.lower()

    candidates = []
    if ckpt_arch == "eagle3":
        # EAGLE3-style checkpoint needs draft_vocab_size in config
        if "llava" in arch or "llama" in arch:
            candidates.append("train/configs/llava1.5_7b_eagle_config.json")
            candidates.append("train/configs/llava1.5_7b_eagle3_config.json")
        if "qwen" in arch:
            candidates.append("train/configs/qwen2.5vl_eagle3_config.json")
    else:
        if "llava" in arch or "llama" in arch:
            candidates.append("train/configs/llava1.5_7b_config.json")
        if "qwen" in arch:
            candidates.append("train/configs/qwen2.5vl_config.json")

    # Keep a safe default ordering for unknown families.
    candidates.extend(
        [
            "train/configs/llava1.5_7b_config.json",
            "train/configs/qwen2.5vl_config.json",
        ]
    )
...
class SpecModel(nn.Module):
    ...
    def __init__(
        ...
        is_eagle3_ckpt=False,
    ):

        super().__init__()
        self.base_model = base_model
        self.config = base_model.config
        self.is_eagle3_ckpt = is_eagle3_ckpt
        ...
        if is_eagle3_ckpt:
            # Use EAGLE3 Model class for eagle3-style checkpoints
            from method.eagle3.cnets import Model as Eagle3Model
            with warnings.catch_warnings():
                warnings.filterwarnings("ignore", message="You are using a model of type")
                self.spec_layer = Eagle3Model(
                    config,
                    load_emb=True,
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        try:
            ...
            # Merge additional shards (model_1.safetensors, model_2.safetensors, ...)
            shard_idx = 1
            while True:
                shard_name = f"model_{shard_idx}.safetensors"
                shard_path = os.path.join(spec_model_path, shard_name)
                if not os.path.exists(shard_path):
                    try:
                        shard_path = hf_hub_download(spec_model_path, shard_name)
                    except Exception:
                        break
                print(f"[eagle] Merging shard: {shard_name}")
                with open(shard_path, "rb") as f:
                    shard_dict = load(f.read())
                for k, v in shard_dict.items():
                    if k not in spec_layer_state_dict:
                        spec_layer_state_dict[k] = v
                shard_idx += 1

        # --- Auto-detect checkpoint architecture ---
        is_eagle3_ckpt = _detect_checkpoint_arch_from_state_dict(spec_layer_state_dict) == "eagle3"
        if is_eagle3_ckpt:
            print(
                f"[eagle] Detected EAGLE3-style checkpoint in '{spec_model_path}'. "
                "Using EAGLE3 Model class with EAGLE1 inference."
            )

        # --- Find config ---
        configpath = os.path.join(spec_model_path, "config.json")
        if not os.path.exists(configpath):
            ckpt_arch_str = "eagle3" if is_eagle3_ckpt else "eagle1"
            fallback = _pick_default_spec_config(base_model_path, ckpt_arch=ckpt_arch_str)
    ...
    def forward(
        ...
        **kwargs,
        ...
        with torch.inference_mode():
            ...
            if self.is_eagle3_ckpt:
                # EAGLE3-style: concatenate first 3 hidden states
                ...
                # hs[0] at image positions contains raw vision features (not text
                # embeddings), which corrupts the spec model's KV cache because
                # the spec model was trained on text-only data.
                ...
                # EAGLE1-style: use last hidden state
