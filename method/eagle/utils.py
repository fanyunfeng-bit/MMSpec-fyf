...
def initialize_tree(
    ...
    **kwargs,
    ...
    # For EAGLE3 + VLM: replace image tokens with pad in the input_ids copy
    # given to topK_genrate so the spec model uses pad embeddings (instead of
    # image-placeholder embeddings) at image positions whose hidden_states have
    # already been zeroed out in SpecModel.forward().
    spec_input_ids = input_ids
    if getattr(model, "is_eagle3_ckpt", False):
        _cfg = getattr(model, "config", None)
        _img_id = getattr(_cfg, "image_token_index",
                          getattr(_cfg, "image_token_id", None)) if _cfg else None
        if _img_id is not None and (input_ids == _img_id).any():
            spec_input_ids = input_ids.clone()
            _pad = getattr(model.tokenizer, "pad_token_id", 0) or 0
            spec_input_ids[spec_input_ids == _img_id] = _pad
