        ...
        if position_ids is None:
            ...
            if past_key_value is not None:
                if len(past_key_value) == 2:
                    ...
                    past_real_len = past_key_value[2]
                    if isinstance(past_real_len, torch.Tensor):
                        past_real_len = int(past_real_len.item())
                    else:
                        past_real_len = int(past_real_len)
                    kv_seq_len += past_real_len
                else:
...
class Model(nn.Module):
    ...
    def topK_genrate(
        ...
        """EAGLE2: Same draft loop as EAGLE1 (correct KV cache), but with:
        1. Threshold-based early stopping at each tree depth
        2. Confidence-aware final token selection
        """
        ...
        for i in range(depth):
            ...
            # EAGLE2: threshold-based early stopping
        ...
        # EAGLE2: confidence-aware token selection
        # Apply threshold to prune low-confidence candidates
        n_candidates = scores_list.shape[0]
        if self.threshold < 0:
            above_threshold = scores_list > self.threshold
            n_above = above_threshold.sum().item()
            effective_tokens = min(total_tokens, max(n_above, top_k))
        else:
            effective_tokens = min(total_tokens, n_candidates)
        
        top_scores = torch.topk(scores_list, effective_tokens, dim=-1)
        ...
        # with Timer("mask"):
        tree_mask = torch.eye(effective_tokens + 1).bool()
        tree_mask[:, 0] = True
        ...
        leaf_num = effective_tokens - noleaf_num
        ...
        for i in range(effective_tokens + 1):
