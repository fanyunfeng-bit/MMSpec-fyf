...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    ...
    def _update_causal_mask(
        ...
        if hasattr(self, "tree_mask") and self.tree_mask is not None:
            ...
            if os.environ.get("MEDUSA_DEBUG", "0") == "1":
                print(f"[MEDUSA] [_update_causal_mask] tree_mask applied: "
                      f"tree_len={tree_len}, causal_mask.shape={causal_mask.shape}, "
                      f"past_seen_tokens={past_seen_tokens}, seq_len={sequence_length}", flush=True)
                # Show which positions are visible to each tree token
                tree_region = causal_mask[0, 0, -tree_len:, :]
                nonmasked_counts = (tree_region > torch.finfo(tree_region.dtype).min + 1).sum(dim=-1)
                print(f"[MEDUSA]   visible_positions_per_tree_token={nonmasked_counts.int().tolist()}", flush=True)
            causal_mask[:, :, -tree_len:, -tree_len - 1 : -1][
                tree_mask == 0
            ] = causal_mask.min()
            if os.environ.get("MEDUSA_DEBUG", "0") == "1":
                tree_region = causal_mask[0, 0, -tree_len:, :]
                nonmasked_after = (tree_region > torch.finfo(tree_region.dtype).min + 1).sum(dim=-1)
                print(f"[MEDUSA]   after tree_mask: visible_per_token={nonmasked_after.int().tolist()}", flush=True)
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        config.max_position_embeddings = 8192  # [MODIFIED] increased from 4096 to handle long image-token sequences
        super().__init__(config)
    ...
    def forward(
        ...
        if (
            ...
            if os.environ.get("MEDUSA_DEBUG", "0") == "1":
                print(f"[MEDUSA] [Qwen2.5VL.forward] cache_position computed: "
                      f"past_len={past_len}, seq_len={seq_len}, "
                      f"cache_position={cache_position.tolist()}", flush=True)
