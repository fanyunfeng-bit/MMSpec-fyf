...
class Model(nn.Module):
    def __init__(self, config, load_emb=False, path=None, bias=True, total_tokens=63, depth=5, top_k=8, threshold=1.0, train_embed=False, decouple=False):
        ...
        from .ea_llama_model import LlamaDecoderLayer
        from .ea_qwen2vl_model import Qwen2VLDecoderLayer, Qwen2VLRotaryEmbedding
    ...
    def forward(
        ...
        # with torch.no_grad():
        from .utils import temp_cache

        has_image_tokens_num = image_tokens_num is not None and len(image_tokens_num) > 0
        ...
        if any(p.requires_grad for p in self.embed_tokens.parameters()):
            ...
            for batch_idx in range(input_ids.size(0)):
                ...
                if -200 in single_input_ids and has_image_tokens_num:
                ...
                elif 151652 in single_input_ids and has_image_tokens_num:
            ...
            if -200 in input_ids and has_image_tokens_num:    # llava
            ...
            elif 151652 in input_ids and has_image_tokens_num:    # qwen2_vl
        ...
        if self.decouple: # train
            for i in range(input_ids.shape[0]):
                ...
                elif 151652 in input_ids[i] and has_image_tokens_num:
            ...
            if -200 in input_ids and has_image_tokens_num:
            ...
            elif 151652 in input_ids and has_image_tokens_num:
    ...
    def topK_genrate(self, hidden_states, input_ids, head, logits_processor, inputs_embeds=None):
        ...
        if inputs_embeds is not None and inputs_embeds.shape[1] > input_ids.shape[1]:
            # Generic multimodal expansion: inputs_embeds already contains expanded visual tokens.
            image_extra_tokens = int(inputs_embeds.shape[1] - input_ids.shape[1])
            len_posi += image_extra_tokens
        elif -200 in input_ids:  # Legacy LLaVA marker path.
            image_extra_tokens = 575
            len_posi += image_extra_tokens

        self.reset()

        use_stable_kv = hasattr(self, "stable_kv") and self.stable_kv is not None
        if use_stable_kv:
            kv_len = self.stable_kv[0][0].shape[2]
            if image_extra_tokens > 0:
                kv_len -= image_extra_tokens
            # Guard against KV/image-length drift in multimodal scenarios.
            if kv_len < 0 or kv_len >= input_ids.shape[1]:
                use_stable_kv = False
                self.stable_kv = None

        if use_stable_kv:
            input_tail = input_ids[:, kv_len:]
            # Keep IDs/hidden states aligned when KV length math drifts by 1 token.
            tail_len = hidden_states.shape[1]
            if input_tail.shape[1] != tail_len:
                input_tail = input_ids[:, -tail_len:]
            out_hidden, past_key_values = self(
                hidden_states,
                input_ids=input_tail,
                past_key_values=self.stable_kv,
                use_cache=True,
                # Visual context is already captured in stable_kv. Passing the full
                # multimodal inputs_embeds here can desynchronize seq lengths.
                inputs_embeds=None,
            )
        else:
            image_tokens_num = []
            if inputs_embeds is not None:
                if image_extra_tokens > 0:
                    # `image_extra_tokens` already equals the visual-embedding span length for
                    # the compact marker input used by MSD.
                    image_tokens_num = [image_extra_tokens]
                elif -200 in input_ids:
