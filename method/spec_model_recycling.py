...
class SpecModel(nn.Module):
    ...
    def specgenerate(
        ...
        **kwargs,
        ...
        # Keep Recycling and anti-loop parameters fixed in spec_model.
        ...
        kwargs.pop("matrix_top_k", None)
        kwargs.pop("draft_len", None)
        ...
        # ===== Main decode loop =====
        # print(f"[RECYCLING] Entering main decode loop (max_length={max_length})...")
        # idx = 0
        ...
        for idx in range(max_length):
            ...
            if generated_so_far >= max_new_tokens:
                # print(f"[RECYCLING] [ITER {idx}] BREAK: max_new_tokens reached "
                #       f"({generated_so_far} >= {max_new_tokens})")
                break
            
            # Check EOS
            if self.tokenizer.eos_token_id is not None and self.tokenizer.eos_token_id in input_ids[0, input_len:]:
                # print(f"[RECYCLING] [ITER {idx}] BREAK: EOS token found in generated tokens")
                break
        ...
        # print(f"[RECYCLING] GENERATION COMPLETE")
