...
def _collect_stop_token_ids(tokenizer, model):
    ...
    # Treat any tokenizer special token as a valid stopping boundary.
    stop_ids.update(_normalize_token_ids(getattr(tokenizer, "all_special_ids", None)))
    return stop_ids
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if not os.path.exists(configpath):
            if "Llava" in Type:
                fallback = "train/configs/llava1.5_7b_vispec_config.json"
            else:
                fallback = "train/configs/qwen2.5vl_vispec_config.json"
        ...
        **kwargs,
        ...
        for idx in range(max_length):
            ...
            if getattr(self.base_model.config, "_attn_implementation", None) == "eager":
                # Safe eager path: disable multi-token acceptance to avoid committing
                # misaligned draft branches under eager attention implementation.
                accept_length = 0
            ...
            (
                ...
                past_key_values=past_key_values,
                strict_next_token=(
                    getattr(self.base_model.config, "_attn_implementation", None)
                    == "eager"
                ),
                # inputs_embeds,
                # embed_weights,
