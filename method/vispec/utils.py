...
def tree_decoding(
    ...
    kv_base_len = input_ids.shape[1]
    if current_length_data is not None and current_length_data.numel() > 0:
        kv_base_len = max(kv_base_len, int(current_length_data.max().item()))
    position_ids = tree_position_ids + kv_base_len
...
def update_inference_inputs(
    ...
    strict_next_token=False,
    # inputs_embeds=None,
    # embed_weights=None,
    # image_mask=None,
):
    prev_input_len = input_ids.shape[1]
    if cache_base_len is None:
        kv_base_len = prev_input_len
        if current_length_data is not None and current_length_data.numel() > 0:
            kv_base_len = max(kv_base_len, int(current_length_data.max().item()))
    else:
        kv_base_len = int(cache_base_len)
    # Map the best candidate indices to the original indices in the sequence
    select_indices = (
        retrieve_indices[best_candidate, : accept_length + 1] + kv_base_len
    )
    ...
    for past_key_values_data in past_key_values_data_list:
        ...
        dst = past_key_values_data[
            ..., kv_base_len : kv_base_len + tgt.shape[-2], :
        ]
        # Copy relevant past information from the source to the destination
        dst.copy_(tgt, non_blocking=True)

    # Update the current length tensor (currently only support batch size is 1)
    current_length_data.fill_(kv_base_len + tgt.shape[-2])
    ...
    # token=token[None,None]
    prob = sample_p
    next_hidden_state = accept_hidden_state_new
    if strict_next_token and past_key_values is not None:
        # Calibrate boundary token with one exact target-model step in eager mode.
        # Restore cache cursors afterwards so speculative cache bookkeeping remains consistent.
        saved_lengths = (
            current_length_data.clone() if current_length_data is not None else None
        )
        reset_tree_mode(model)
        exact_out = model.base_model(
        ...
        if saved_lengths is not None:
            current_length_data.copy_(saved_lengths)
        exact_logits = exact_out.logits[:, -1, :]
        if logits_processor is not None:
            exact_logits = logits_processor(None, exact_logits)
            exact_probs = torch.softmax(exact_logits, dim=-1)
            token = torch.multinomial(exact_probs, 1)
            prob = exact_probs
        else:
            token = torch.argmax(exact_logits, dim=-1, keepdim=True)
            prob = exact_logits
    else:
        if logits_processor is not None:
