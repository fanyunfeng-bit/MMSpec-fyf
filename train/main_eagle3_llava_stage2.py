...
parser = argparse.ArgumentParser(description='EAGLE3 Stage 2: Multimodal Training for LLaVA-1.5')
...
class MultimodalDataset(Dataset):
    """Dataset for multimodal (image + text) EAGLE3 training on LLaVA-1.5."""
    ...
    def __getitem__(self, idx):
        ...
        # Build text prompt with <image> token for LLaVA processor
        text_parts = []
        for j, sentence in enumerate(source):
            ...
            if role == "user":
                if "<image>" in content:
                    # Replace <image> with the processor's image token
...
if global_rank == 0:
    print("EAGLE3 Stage 2: Multimodal training for LLaVA-1.5")
