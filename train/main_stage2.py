...
from train.model.cnets_multimodal import Model
...
def _find_weight_key(weight_map, candidates, suffix):
    ...
    for key, value in weight_map.items():
        if key.endswith(suffix):
            return key, value
    raise KeyError(f"Cannot find weight key with suffix '{suffix}'.")

try:
    with open(os.path.join(args.basepath, "model.safetensors.index.json"), "r") as f:
        index_json = json.loads(f.read())
        head_key, head_path = _find_weight_key(
...
if args.loadpath:
    ...
    import glob
    if os.path.isdir(args.loadpath):
        # Handle accelerate's sharded safetensors directory
        safetensor_files = sorted(glob.glob(os.path.join(args.loadpath, "*.safetensors")))
        if safetensor_files:
            state_dict = {}
            for sf in safetensor_files:
                state_dict.update(load_file(sf))
            print(f"Loaded {len(state_dict)} keys from {len(safetensor_files)} sharded safetensors files")
        else:
            # Try pytorch_model.bin
            bin_files = sorted(glob.glob(os.path.join(args.loadpath, "pytorch_model*.bin")))
            state_dict = {}
            for bf in bin_files:
                state_dict.update(torch.load(bf, map_location="cpu"))
            print(f"Loaded {len(state_dict)} keys from {len(bin_files)} bin files")
    else:
        try:
            state_dict = load_file(args.loadpath)
        except:
            state_dict = torch.load(args.loadpath, map_location="cpu")
...
for epoch in range(num_epochs + 1):
    ...
    for batch_idx, data in enumerate(tqdm(train_loader)):
        ...
        with accelerator.accumulate(model):
            optimizer.zero_grad()
            idx = data.get("input_ids")
            embs = data.get("inputs_embeds")
            predict = model(data["hidden_states"], input_ids=idx, inputs_embeds=embs, attention_mask=data["attention_mask"])
            with torch.no_grad():
    ...
    if (epoch + 1) % train_config["save_freq"] == 0:
        ...
        for batch_idx, data in enumerate(tqdm(test_loader)):
            with torch.no_grad():
                if batch_idx < 10 and data.get("input_ids") is not None:
