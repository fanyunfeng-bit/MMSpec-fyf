...
"""EAGLE3 draft model for LLaVA-1.5-7B target model."""
import math
...
class Model(nn.Module):
    def __init__(self, config, ds_config, training_config, load_head=False, load_emb=True, path=None):
        ...
        # Setting dschf to allow efficient ZeRO-3 usage between hf and ds.
        ...
        self.length = 7
        # Load LLaVA-1.5 as target model with hidden states output
        ...
        if not load_emb:
            ...
            # Load embeddings from LLaVA's language model
            try:
        ...
        if not os.path.exists("cache.pt"):
            ...
            def preprocess_function(examples):
                ...
                for i in range(len(examples['id'])):
                    ...
                    # Mask out system and user turns, keep only assistant turns
                    sep = "</s>"
    ...
    def dataprepare(self, input_ids, attention_mask, loss_mask, pixel_values=None):
        ...
        # Pass output_hidden_states=True to get all layer hidden states
        kwargs = dict(
            input_ids=input_ids, 
            attention_mask=attention_mask,
            output_hidden_states=True
        )
        # Add multimodal inputs if present (Stage 2)
        if pixel_values is not None:
            kwargs["pixel_values"] = pixel_values.to(device)
        outs = self.target_model(**kwargs)
        # Access hidden states from the language model
        # hidden_states[0] = embeddings, [1] = after layer 0, [2] = after layer 1
