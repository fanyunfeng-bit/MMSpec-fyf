...
class Model(nn.Module):
    def __init__(self, config, ds_config, training_config, load_head=False, load_emb=True, path=None):
        ...
        self.length = 7
        # Load Qwen2.5-VL as target model with hidden states output
        self.target_model = AutoModelForImageTextToText.from_pretrained(
            path, torch_dtype=torch.bfloat16, output_hidden_states=True
        )
        ...
        if not load_emb:
            ...
            # Load embeddings from the VLM's language model
            try:
                lm = self.target_model.language_model if hasattr(self.target_model, 'language_model') else self.target_model.model
            ...
            def preprocess_function(examples):
                ...
                for i in range(len(examples['id'])):
                    ...
                    for pi, part in enumerate(parts):
                        if pi == 0:
                            # Everything before first assistant turn
    ...
    def dataprepare(self, input_ids, attention_mask, loss_mask, pixel_values=None, image_grid_thw=None):
        ...
        # For VLM models, pass output_hidden_states=True to get all layer hidden states
        kwargs = dict(
            input_ids=input_ids, 
            attention_mask=attention_mask,
        ...
        # Access hidden states: for VLM, hidden_states come from the language model
