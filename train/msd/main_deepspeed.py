...
from train.msd.model.cnets import Model
from train.msd.model.configs import EConfig, Qwen2VLConfig
...
def main():
    ...
    parser.add_argument('--resume_model', type=str, default=None, help="Path to a saved MSD checkpoint dir to resume from")
    parser.add_argument('--start_epoch', type=int, default=0, help="Epoch index to start/resume from (0-based)")
    parser.add_argument('--num_epochs', type=int, default=None, help="Override total number of epochs")
    ...
    train_config = {
        ...
        "is_warmup": True,
        "num_epochs": args.num_epochs if args.num_epochs is not None else 20 * 2,
    ...
    class CustomDataset(Dataset):
        ...
        @staticmethod
        def _build_legacy_qwen2vl_input_ids(image_mask: torch.Tensor) -> torch.Tensor:
            input_ids = torch.zeros_like(image_mask, dtype=torch.long)
            active = (image_mask > 0).nonzero(as_tuple=False).flatten()
            if active.numel() == 0:
                return input_ids

            # Legacy visual data stores image spans as a contiguous boolean mask.
            start = int(active.min().item())
            end = int(active.max().item())
            input_ids[start : end + 1] = 151655
            if start > 0:
                input_ids[start - 1] = 151652
            else:
                input_ids[0] = 151652
            return input_ids
        ...
        def __getitem__(self, index):
            ...
            if "input_ids" in data:
                ...
                input_ids = self._build_legacy_qwen2vl_input_ids(data["image_mask"])[:train_config["max_len"]][None, :]
            else:
                raise KeyError("Training sample requires `input_ids` or legacy `image_mask`.")
    ...
    if args.start_epoch > 0:
        traindataset.epoch_now = args.start_epoch + 1
        testdataset.epoch_now = args.start_epoch + 1
        traindataset.update_ratio()
        testdataset.update_ratio()
    ...
    if args.resume_model:
        resume_bin = os.path.join(args.resume_model, "pytorch_model.bin")
        if not os.path.exists(resume_bin):
            raise FileNotFoundError(f"resume checkpoint not found: {resume_bin}")
        state_dict = torch.load(resume_bin, map_location="cpu")
        ...
        if rank == 0:
            print(f"Loaded resume checkpoint from {args.resume_model}")
